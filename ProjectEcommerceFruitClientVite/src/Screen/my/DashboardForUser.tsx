...
export default observer(function DashboardForUser() {
  ...
  //     const ordersByMonth = order.filter(x=>x.status === 1).reduce((acc: any, currentOrder) => {
  ...
  useEffect(() => {
    if (order) {
      ...
      setYearOptions(
        years.map((year: any) => ({ value: year, label: year }))
      );

      const ordersByMonth = order
        .filter((x) => x.status === 1)
        .reduce((acc: any, currentOrder) => {
          const orderYear = dayjs(currentOrder.createdAt).year();
          if (orderYear !== selectedYear) return acc;

          const month = dayjs(currentOrder.createdAt).format("MMMM"); // ใช้ dayjs เพื่อแสดงเดือนเป็นภาษาไทย
          const orderYear = moment(currentOrder.createdAt).year();
          if (orderYear !== selectedYear) return acc;
  ...
  const generateExcel = () => {
    ...
    worksheet.addRow({ item: "ยอดเงินทั้งหมดที่ซื้อ", value: totalPrice, unit:"บาท"});
  
    worksheet.addRow({ item: "จำนวนสินค้าที่ซื้อ", value: totalQuantity, unit:"ชิ้น"});
  
    worksheet.addRow({ item: "ยอดคำสั่งซื้อที่สำเร็จ", value: totalOrderSuccess, unit:"ครั้ง"});
  
    worksheet.addRow({ item: "ยอดคำสั่งซื้อที่ยกเลิก", value: totalOrderFailed, unit:"ครั้ง" });
  
    worksheet.addRow({ item: "ยอดคำสั่งซื้อในแต่ละเดือน", value: "" });
    monthlyOrderData.forEach(monthData => {
      worksheet.addRow({ item: monthData.month, value: monthData.total, unit:"บาท" });
    });
  
    worksheet.addRow({ item: "สัดส่วนการใช้จ่ายในแต่ละหมวดหมู่สินค้า", value: "" });
    pieChartData.forEach(categoryData => {
      worksheet.addRow({ item: categoryData.name, value: categoryData.value, unit:"ชิ้น" });
    });
  ...
  >
    <div className="flex flex-row items-center justify-center">
      ...
      <span className="font-bold text-gray-600"> {totalOrderFailed} </span>
    </div>

    <div className="mt-2 text-sm text-gray-400">ยอดคำสั่งซื้อที่ยกเลิก</div>
  </a>
